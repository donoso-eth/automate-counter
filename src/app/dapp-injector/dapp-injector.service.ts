...
import { OZIncreaseContract} from 'src/assets/contracts/interfaces/OZIncreaseContract';
import { AngularContract } from './classes';

import OZIncreaseCounterMetadata from '../../assets/contracts/oz_increase_contract_metadata.json';
...
export class DappInjector implements OnDestroy {
  ...
  DAPP_STATE:IDAPP_STATE<OZIncreaseContract> = {
...
async localWallet(index:number) {
 ...
 private  async walletInitialization() {
  ...
  private async contractInitialization() {
    ...
    const contract = new AngularContract<OZIncreaseContract>({
      metadata: OZIncreaseCounterMetadata,
